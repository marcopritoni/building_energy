...
# fill missing variables or cut outliers with mean data
    data.loc[((data.chw) < cond_chw_min) | (
        (data.chw) > cond_chw_max), "chw"] = data.loc[:, 'chw'].mean()
...
# PLOT MODEL
        plot_compare(compare_train, compare_train.index.min(),
                     compare_train.index.max())
        plot_compare(compare_val, compare_val.index.min(),
                     compare_val.index.max())
        plot_compare(compare_sav, compare_sav.index.min(),
                     compare_sav.index.max())
# PLOT SAVINGS
...
def year_savings():
    ...
    time_res = "M"
    fuel = ["chw", "elec", "steam"]
    now = pd.to_datetime('7,1,2016', format='%m,%d,%Y',
                         errors='coerce', dayfirst=False)  # data.index.max()
    beginning = now - pd.Timedelta("366 days")
    end = pd.to_datetime('7,1,2016', format='%m,%d,%Y',
    ...
    tot = {}
    for targ in fuel:
        tot[targ] = (data_annual[targ] * saving_perc.T.loc[:, targ].mean() /
                     100 * cost[targ]).sum() + saving_cost.T.loc[:, targ].sum()
    tot = pd.Series(tot)
    print tot  # .sum()
    return tot.sum()
year_savings()
...
data_clean.loc["2013", ["chw", "oat"]].plot(
    kind="scatter", x="oat", y="chw", figsize=(18, 5))
data_clean.loc[:, ["chw", "oat"]].to_csv(
    "data/clean/Ghausi_M&V_Data_chw_clean.csv")

cond = ~((data["oat"] < 65) & (data["steam"] == 0))
data_clean = data.loc[cond, :]
data_clean.loc["2015", ["steam", "oat"]].plot(figsize=(18, 5))
data_clean.loc["2015", ["steam", "oat"]].plot(
    kind="scatter", x="oat", y="steam", figsize=(18, 5))
...
data_clean.loc["04-01-2016":, ["elec", "oat"]
               ].plot(kind="scatter", x="oat", y="elec", figsize=(18, 5))
data_clean.loc[:, ["elec", "oat"]].to_csv(
    "data/clean/Ghausi_M&V_Data_elec_clean.csv")
...
cond = ~((data["oat"] < 65) & (data["steam"] == 0))
data_clean = data.loc[cond, :]
data_clean.loc[:, ["steam", "oat"]].groupby(
    pd.TimeGrouper(freq='M')).mean().plot(figsize=(18, 5))
data_clean.loc[:, ["steam", "oat"]].groupby(pd.TimeGrouper(freq='M')).mean(
).plot(kind="scatter", x="oat", y="steam", figsize=(18, 5))
data_clean.loc[:, ["steam", "oat"]].to_csv(
    "data/clean/Ghausi_M&V_Data_steam_clean.csv")
