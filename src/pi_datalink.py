...
class PIDatalink(object):
    ...
    def _parse_TS(self, response, Web_ID, _label):
        ...
        if response:
            ...
            ts = self._utc_to_local(ts, 'America/Los_Angeles')  # Adjusting timestamp
...
#        print Web_ID_string
        ...
        if _end != "t" and _end != "y" and _end != "*":
            _end = self._local_to_utc(_end, 'America/Los_Angeles')  # adjusting time because requested timeframe is based on utc time            
    ...
    def search_by_path (self,
        ...
        # call request library
        response = req.get(query_string, parms)
        logging.info(response)  # Should be getting a Response of 200 if successful
        # parse result and return it
    ...
    def get_webID_by_point (self,
        ...
        if len (pointList) > 1:
            logging.warning("the query returned more than one WebID n=%d, \
